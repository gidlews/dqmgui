...
# -----------------------------------------------------------------
  ...
  def jsonfairy(self, *args, **kwargs):
    """General session-independent access path for json 
    representation of plot. The first subdirectory argument contains 
    the name of the "source json hook" able to handle the json request.
    In case of value of argument 'formatted' = true insread of pure JSON
    whole HTML page is returned.
    The rest of the processing is given over to the hook."""
    try:
      if len(args) >= 1:
        for s in self.sources:
          if getattr(s, 'jsonhook', None) == args[0]:
            data = s.json(*args[1:], **kwargs)
