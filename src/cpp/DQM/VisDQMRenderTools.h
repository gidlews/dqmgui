...
{
	string result;
	if(arrayString.at(arrayString.size()-1) == ',') // if there is useless comma at the end then remove it
...
/* Following method return json containing all bins data and (due to code optimization) part of statistics: integral. */
static string binsToArray(const TH1* const h)
{
	...
	string errorList = "";
	Double_t sum =0;
	const Double_t defWidth = (h->GetXaxis()->GetXmax() - h->GetXaxis()->GetXmin())/h->GetNbinsX();
	bool isWidthDef = true;
	for(int i=h->GetXaxis()->GetFirst(); i!=h->GetXaxis()->GetLast()+1; ++i)
	{
	...
	if(isWidthDef)
		...
		return integral + "," + contentList + "," + arrayToJson(widthList, "width")+ ","+ errorList;
}
/* Following method return json containing all bins data and (due to code optimization) element of histogram statistics: integral. */
static string binsToArray(const TH2* const h)
{
	...
	const Double_t defWidthX = (h->GetXaxis()->GetXmax() - h->GetXaxis()->GetXmin())/h->GetNbinsX();
	const Double_t defWidthY = (h->GetYaxis()->GetXmax() - h->GetYaxis()->GetXmin())/h->GetNbinsY();
...
template <typename T>
static string optionalNumericValueToJson(const char* const name, T value, T defaultValue = 0, bool avoidComma = false)
{
...
template <typename T>
static string optionalTextValueToJson(const char* const name, T value, T defaultValue = "", bool avoidComma = false)
{
...
{
	string result = "";
	result += optionalTextValueToJson<const char* const>("title",h->GetTitle(), NULL, true);
	if(h->GetNbins() > 1)
	{
		if(result != "")
			result +=",";
...
/*			.arg(statWithErrorToJson(hist, "kurtosis"))
			.arg(statWithErrorToJson(hist, "skewness"))*/;
	return result;
}
