  ...
  bool
   makeJson(std::string &jsonData,
	  ...
	  const std::string &spec)
     {
       ...
       proto.id = Time::current().ns();
       proto.imagespec = spec;
       Image &img = proto; //change to json
       ...
       img.busy = true;
       img.inuse++;
       requestimg(img,jsonData,true); //change to requestdata
  ...
  sessionZoomConfig(const py::dict &session)
    {
      ...
      return StringFormat("'zoom':{'show':%1,'x':%2,'y':%3,'w':%4,'h':%5,'jsonmode':%6,'jx':%7,'jy':%8,'jw':%9,'jh':%10}")
	.arg(show).arg(x).arg(y).arg(w).arg(h).arg(jsonmode).arg(jx).arg(jy).arg(jw).arg(jh);
    }
...
BOOST_PYTHON_MODULE(Accelerator)
{
  ...
  py::class_<VisDQMArchiveSource, shared_ptr<VisDQMArchiveSource>,
    ...
    .def("_json", &VisDQMArchiveSource::getjson)
